...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and user.password == password:
            ...
            return jsonify({'error': 'Неверный Логин или пароль, попробуйте снова'})
...
def save_match():
    ...
    if connection:
        try:
            ...
            # Send notifications to the staff members
            staff_ids = [
                data.get('operator_id'),
                data.get('director_id'),
                data.get('commentator_id'),
                data.get('photograph_id'),
                data.get('employee1_id'),
                data.get('employee2_id'),
                data.get('employee3_id')
            ]

            staff_ids = [staff_id for staff_id in staff_ids if staff_id]

            if staff_ids:
                staff_query = "SELECT staff_id, staff_name, telegram_chat_id FROM staff WHERE staff_id IN (%s)" % ','.join(['%s'] * len(staff_ids))
                cursor.execute(staff_query, staff_ids)
                staff_members = cursor.fetchall()

                for member in staff_members:
                    message = f"Вы добавлены на новый матч: {data['match_date']} {data['match_time']} - {data['comment']}"
                    send_telegram_message(member['telegram_chat_id'], message)
